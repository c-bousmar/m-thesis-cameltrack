...
output_file = os.path.join(
    info_folder, f"IDs_correspondence_{video_name}.png"
)

FRAME_SCALE = 0.5
BAR_HEIGHT = 0.4
FN_BAR_THICKNESS = 0.20      # FN plus fin
EMPTY_BAR_THICKNESS = 0.02   # pour les frames vides
ID_S_FRAMES = []             # frames à indiquer sur l'axe en bas

# ============================================================
# LOAD TRACKLAB COLORS
# ============================================================
BASE_COLORS = np.load(colors_file)
cmap_size = len(BASE_COLORS)

# ============================================================
# LOAD CSV + DETECT FRAME RANGE
# ============================================================
...
# COLOR MAP
# ============================================================
unique_pred_ids = sorted({int(x) for x in pivot.flatten() if not np.isnan(x) and x > 0})
pid_to_color = {pid: BASE_COLORS[(pid-1)%cmap_size] for pid in unique_pred_ids}
FN_COLOR = (0,0,0)
EMPTY_COLOR = (0,0,0)
...
ax.set_xlim((frame_start-0.5)*FRAME_SCALE, (frame_end+0.5)*FRAME_SCALE)
    ...
    for i, pid in enumerate(preds):
        if np.isnan(pid):
            ...
            if cur_pid is not None:
                ...
                if cur_pid == -1:
                    ax.add_patch(Rectangle(
                        ...
                        width, FN_BAR_THICKNESS,
                        facecolor=FN_COLOR, edgecolor='none', zorder=2
                    ))
                else:
            ...
            ax.add_patch(Rectangle(
                ...
                FRAME_SCALE, EMPTY_BAR_THICKNESS,
                facecolor=EMPTY_COLOR, edgecolor='none', zorder=1
            ))
            continue
...
for f in ID_S_FRAMES:
    ...
    ax.text(x, y_bottom, str(f), ha='center', va='top', fontsize=10, rotation=90, color='red', zorder=5)
