...
seq_dirs = [d for d in os.listdir(dataset_folder)
...
for TE_NAME in TE_LIST:
    ...
    tokens_array = np.asarray(tokens_list, dtype=np.float32)
    ...
    for f_id in range(frame_start, frame_end + 1):
        ...
        mask = frame_ids <= f_id
        track_mask = mask & (types == 1)
        det_mask   = mask & (types == 0)
        ...
        ax.set_title(
            f"{method_name.upper()} — {TE_NAME.upper()} encoder\n"
            f"{video_name} — frames ≤ {f_id}"
        )
